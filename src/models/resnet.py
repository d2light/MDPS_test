...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        feature_a = self.layer1(x)  # torch.Size([4, 256, 64, 64])
        feature_b = self.layer2(feature_a) # torch.Size([4, 512, 32, 32])
        feature_c = self.layer3(feature_b) # torch.Size([4, 1024, 16, 16])
        feature_d = self.layer4(feature_c) # torch.Size([4, 2048, 8, 8])
